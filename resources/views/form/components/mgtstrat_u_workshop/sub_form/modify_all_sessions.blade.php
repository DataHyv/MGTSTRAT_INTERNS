...
<div class="customized-engagement" id="main">
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>MGTSTRAT-U Workshop Sessions</h3>
            </div>                
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-content">
            {{-- card body --}}
            <div class="card-body">
                <form class="form form-horizontal multisteps-form__form" action="{{ route('form/workshopEngagement/sub-form/savebatchsessions') }}" method="POST" autocomplete="off"> 
                @csrf 
                ...
                <div class="alert alert-light row" role="alert" style="border-left: 3px solid #818182;">    
                    <div class="col-lg-6 col-md-6"> 
                        ...
                        <a href="{{ url('update_workshop_eng/' . $parentData[0]->id ) }}" target="_blank">
                            <h5 id="engType" style="text-decoration: underline">
                                {{ ($parentData[0]->engagement_title == '- NOT LISTED -') ? $parentData[0]->engagement_title_not_listed : $parentData[0]->engagement_title; }}
                            </h5>
                        </a>
                    </div>
                    <div class="col-lg-6 col-md-6"> 
                        <span style="font-size:10pt">Form ID</span>
                        <h5 id="paxhNo">{{ $parentData[0]->workshop_eng_form_id }}</h5>
                                ...
                                <!------------------- TABLE HEAD ----------------------->
                                    <thead class="th-blue-grey">
                                        <tr class="text-center">
                                            ...
                                            <th colspan="3">Consulting/Design</th>  
                                    ...
                                    <thead class="th-blue-grey-lighten">
                                        <tr class="text-center text-sm">
                                            ...
                                            <th class="p-1" style="min-width: 180px">Intelligence</th>  
                                            ...
                                            <th class="p-1" style="min-width: 200px">Customization Fee<h6><small>[Roster]</small></h6></th>                                              
                                            ...
                                            <th class="p-1" style="min-width: 200px">Moderator <h6><small>[Roster]</small></h6></th>  
                                            <th class="p-1" style="min-width: 200px">Producer <h6><small>[Roster]</small></h6></th>                                              
                                ...
                                <tbody>
                                    <tr class="th-blue-grey-lighten-2">
                                        <td class="p-1">
                                            ...
                                            <a href="{{ url('form/workshopEngagement/sub-form/' . $subInfo_data->id) }}" target="_blank" class="text-primary text-link" title="View Record"> 
                                        ...
                                        <td class="p-1">
                                            <!-- <select class="form-select" name="cluster[]" disabled>
                                                @foreach ($cluster as $cluster_data)
                                                    @if($subInfo_data->cluster == $cluster_data->name)
                                                        <option selected>{{$cluster_data->name}}</option>
                                                    @else
                                                        <option>{{$cluster_data->name}}</option>
                                                    @endif
                                                
                                                @endforeach
                                            </select> -->
                                            <input type="text" class="form-control core_area" value="{{$subInfo_data->cluster}}" placeholder="Intelligence" name="cluster[]" readonly>
                                        </td>   
                                        <td class="p-1">
                                            <!-- <select class="form-select" name="core_area[]" disabled>
                                                @foreach ($coreArea as $coreArea_data)                                                            
                                                    @if($subInfo_data->core_area == $coreArea_data->name)
                                                        <option selected>{{$coreArea_data->name}}</option>
                                                    @else
                                                        <option>{{$coreArea_data->name}}</option>
                                                    @endif
                                                @endforeach
                                            </select> -->
                                            <input type="text" class="form-control core_area" value="{{$subInfo_data->intelligence}}" placeholder="Intelligence" name="core_area[]" readonly>
                                        </td>   
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Sales')
                                                    ...
                                                    <input  type="hidden" value="{{$subCostData_record->hour_fee}}" name="cost_hour_fee[{{ $recordCount }}][]" 
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Referral')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Engagement Manager')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Customization Fee')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                                    value="{{$subCostData_record->rooster}}"
                                                    oninput="filterConsultant(`roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}`,``,`customizationFee`)"
                                                    list="filtered_consultant_list" 
                                                    autocomplete="off">
                                                    <input  type="hidden" value="{{$subCostData_record->consultant_id}}" name="cost_rooster_id[{{ $recordCount }}][]" 
                                                    id="id_roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}">
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Creators Fees')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="table-danger">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Creators Fees')
                                                    <select class="input js-mytooltip form-select @error('') is-invalid @enderror"
                                                        name="cost_hour_fee[{{ $recordCount }}][]" 
                                                        id="cost_hourfee_roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"                                                        
                                                        ...
                                                        <option value="0" {{ $subCostData_record->hour_fee == '0' ? 'selected="selected"' : '' }}
                                                            title="">
                                                            &#8369;0
                                                        </option>
                                                        <option value="500" {{ $subCostData_record->hour_fee == '500' ? 'selected="selected"' : '' }}
                                                            title="">
                                                            &#8369;500
                                                        </option>
                                                        <option value="1000" {{ $subCostData_record->hour_fee == '1000' ? 'selected="selected"' : '' }}
                                                            title="">
                                                            &#8369;1,000
                                                        </option>
                                                        <option value="2000" {{ $subCostData_record->hour_fee == '2000' ? 'selected="selected"' : '' }}
                                                            title="">
                                                            &#8369;2,000
                                                        </option>
                                                    </select>
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Lead Facilitator')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Moderator')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Producer')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->workshop_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Off-Program Fee')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
        ...
        switch(costType) {
            case 'leadConsultant':                
                if(getFee.data('leadconsultant') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('leadconsultant'));  
                }
                break; 
            case 'designer':
                if(getFee.data('designer') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('designer'));  
                }
                break; 
            case 'leadFacilitator':
                if(getFee.data('feeleadfaci') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('feeleadfaci'));  
                }                           
                break; 
            case 'coLead':   
                if(getFee.data('coleadf2f') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('coleadf2f'));  
                }            
                break; 
            case 'alCoach':
                break;
            case 'coFaci':
                if(getFee.data('cofaci') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('cofaci'));  
                }                 
                break; 
            case 'marshal':      
                if(getFee.data('marshal') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('marshal'));  
                }          
                break; 
            case 'moderator':  
                if(getFee.data('moderator') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('moderator'));   
                }              
                break; 
            case 'producer':
                if(getFee.data('producer') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(getFee.data('producer'));     
                }               
                break; 
            case 'customizationFee':
                if(getFee.data('feeleadfaci') != undefined) {
                    $('#cost_hourfee_' + rosterFieldID).val(currency.format(getFee.data('feeleadfaci').replace(/\₱|,/g, "") * 0.75));  
                } 
                break;
            default: 
                break;
