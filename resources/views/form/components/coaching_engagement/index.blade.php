            ...
            <div class="page-title">
                <div class="row">
                    <div class="col-12 col-md-6 order-md-1 order-last">
                        <h3>Coaching Record</h3>
                    </div>
                    <div class="col-12 col-md-6 order-md-2 order-first">
                        <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                            <ol class="breadcrumb bg-transparent">
                                <li class="breadcrumb-item"><a href="{{ route('home') }}">Dashboard</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Coaching Engagement</li>
                            </ol>
                        </nav>
                    </div>
                </div>
            ...
            <section class="section">
                <div class="card mb-5 mt-5">
                    <div class="card-header col-12 d-flex justify-content-left mt-3 mb-3">
                        <a class="btn btn-primary mt-2 mb-2" href="{{ route('form/coaching/newrecord') }}">
                    ...
                    <div class="card-body table-responsive">
                        <table class="table display dt-responsive nowrap text-dark" id="table1">
                            ...
                            <tbody>
                                @foreach ($coachingList as $key => $item)
                                    <tr id="{{ ++$table }}">
                                        <td class="id text-center text-uppercase fw-bold" role="button" data-toggle="modal" data-target="#bigbudgetlist{{ $item->id }}"> {{ $item->coaching_eng_form_id }} </td>
                                        ...
                                        <td class="email text-center fw-bold" role="button" data-toggle="modal" data-target="#bigbudgetlist{{ $item->id }}">
                                            {{ ($item->engagement_title == '- MIXED -') ? $item->engagement_title_not_listed : $item->engagement_title; }}
                                        </td>
                                        ...
                                        <td class="text-center fw-bold text-center">
                                            <a href="{{ url('update_coaching_eng/'.$item->id) }}">
                                            ...
                                            <!-- Delete Coaching Engagement Modal -->
                                            <div class="modal custom-modal fade" id="delete_estimate{{ $item->id }}" role="dialog">
                                                <div class="modal-dialog modal-dialog-centered modal-md">
                                                    <div class="modal-content">
                                                        <div class="modal-header" style="justify-content: center;">
                                                            <h3 class="mb-2 text-center">Coaching</h3>
                                                        </div>
                                                        <div class="modal-body">
                                                            <div class="form-header">
                                                                <h5 class="text-center">Are you sure want to delete <br>
                                                                    <b>{{ ($item->engagement_title == '- MIXED -') ? $item->engagement_title_not_listed : $item->engagement_title; }}</b>?
                                                                </h5>
                                                            </div>
                                                            <form action="{{ route('couching_deleteRecord') }}" method="POST">
                                                                @csrf
                                                                <input type="hidden" name="id" class="e_id" value="">
                                                                <input type="hidden" name="coaching_engagement_forms_id" class="budget_number" value="{{ $item->id }}">
                                                                <input type="hidden" name="engagement_title" value="{{ ($item->engagement_title == '- MIXED -') ? $item->engagement_title_not_listed : $item->engagement_title; }}" readonly>
                                            ...
                                            <div class="modal fade" id="bigbudgetlist{{ $item->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                <div class="modal-dialog modal-lg">
                                                    <div class="modal-content">
                                                            ...
                                                            <table class="table table-light display dt-responsive nowrap tables" id="tables{{ $table }}">
                                                                ...
                                                                <tbody class="text-center appendBatch" style="font-family:Arial, Helvetica, sans-serif">
                                                                    @foreach ($subInfoList as $key => $item2)
                                                                        @if($item->id == $item2->coaching_engagement_forms_id)
                                                                            <tr>
                                                                                <td>
                                                                                ...
                                                                                <td>
                                                                                    <a href="{{ url('form/coachingEngagement/sub-form/' . $item2->id) }}" class="text-success font-18 px-0" title="Add" id="edit">
                                                        ...
                                                        <div class="modal-footer">                                                           
                                                            <a href="{{ url('form/coachingEngagement/sub-form/modify-sessions/' . $item->id) }}" class="btn btn-success active" role="button">
