...
<div class="customized-engagement" id="main">
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>MGTSTRAT-U Webinar Sessions</h3>
            </div>                
        </div>
    </div>
    ...
    <div class="card">
        ...
        <div class="card-content">
            {{-- card body --}}
            <div class="card-body">
                <form class="form form-horizontal multisteps-form__form" action="{{ route('form/webinarEngagement/sub-form/savebatchsessions') }}" method="POST" autocomplete="off"> 
                @csrf 
                ...
                <div class="alert alert-light row" role="alert" style="border-left: 3px solid #818182;">    
                    <div class="col-lg-6 col-md-6"> 
                        ...
                        <a href="{{ url('update_webinar_eng/' . $parentData[0]->id ) }}" target="_blank">
                    ...
                    <div class="col-lg-6 col-md-6"> 
                        ...
                        <h5 id="paxhNo">{{ $parentData[0]->webinar_eng_form_id }}</h5>
                                ...
                                <tbody>
                                    <tr class="th-blue-grey-lighten-2">
                                        <td class="p-1">
                                            ...
                                            <a href="{{ url('form/webinarEngagement/sub-form/' . $subInfo_data->id) }}" target="_blank" class="text-primary text-link" title="View Record"> 
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Sales')
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Referral')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Engagement Manager')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Customization Fee')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Creators Fees')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Lead Facilitator')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Moderator')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1 table-warning">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Producer')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
                                        ...
                                        <td class="p-1">
                                            @foreach($subCostData as $subCostData_record)
                                                @if($subCostData_record->webinar_sub_informations_id ==  $subInfo_data->id && $subCostData_record->type == 'Off-Program Fee')
                                                    <input type="hidden" value="{{$subCostData_record->id}}" name="subcost_id[{{ $recordCount }}][]">
                                                    <input type="text" class="d-none" value="{{$subCostData_record->type}}" name="cost_type[{{ $recordCount }}][]" readonly>
                                                    <input type="text" class="form-control input-table" name="cost_rooster[{{ $recordCount }}][]" 
                                                    id="roster_batch{{ $subInfo_data->batch_number }}session{{ $subInfo_data->session_number }}_subinfo{{$subInfo_data->id}}_subcos{{$subCostData_record->id}}"
